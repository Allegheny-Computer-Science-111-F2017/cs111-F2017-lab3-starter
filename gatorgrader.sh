...
if [ "$CHECK" = true ]; then
  ...
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/main/java/labthree/ \
                                     --checkfiles TipCalculator.java --singlecomments 11 --multicomments 2 --language Java
  determine_exit_code $?
  # --> GatorGrader CHECK: the "new Date()" fragment exists in the code at least once
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/main/java/labthree \
                                     --checkfiles TipCalculator.java --fragments "new Date()" --fragmentcounts 1
  determine_exit_code $?
  # --> GatorGrader CHECK: at least 4 "println()" fragment exists in the code
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/main/java/labthree \
                                     --checkfiles TipCalculator.java --fragments "println(" --fragmentcounts 9
  determine_exit_code $?
  ...
  determine_exit_code $?
  python3 gatorgrader/gatorgrader.py --nowelcome --commands "gradle run" --fragments "28.75" --languages Java
  determine_exit_code $?
